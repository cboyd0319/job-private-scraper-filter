...
- Current state: Milestones 0 through 6 and Gates 0 through 4 are passing.
  ...
  Desktop startup now owns the fixed platform app-data artifact path, and database startup reconciles interrupted
  reviewed tasks before returning services. Product-target evaluation execution, production publisher trust,
  active-artifact startup reconciliation, native source-pack drop,
